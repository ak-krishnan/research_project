...
def generate_markdown_report():
    ...
    phase2 = st.session_state.verification_report or "No Phase 2 data."
    phase1_raw = json.dumps(st.session_state.raw_json, indent=2) if st.session_state.raw_json else "No Phase 1 data."
    phase1_scout = st.session_state.security_report or "No scout data."

    # Add phase statistics
    phase1_count = st.session_state.get("phase1_count", 0)
    phase1_deduped = st.session_state.get("phase1_deduped", 0)

    ticks = "```"
    report = f"# RepoMind 2-Phase SAST Audit\n**Target Repository:** {repo}\n\n---\n\n"
    report += f"## 📊 Pipeline Statistics\n"
    report += f"- **Phase 1 Raw Findings:** {phase1_count}\n"
    report += f"- **Phase 1 After Dedup:** {phase1_deduped}\n"
    report += f"- **Phase 2 (RAG Verified):** see below\n\n---\n\n"
    report += f"## ✅ Phase 2: RAG-Augmented Context Verification\n{phase2}\n\n---\n\n"
    report += f"## ⚙️ Phase 1: Deterministic Scan + Triage\n### Scout Summary\n{phase1_scout}\n\n### Raw Semgrep Output\n{ticks}json\n{phase1_raw}\n{ticks}\n"
    return report
...
def generate_csv_report():
    ...
    # Prepare enhanced metrics with phase information
    output = io.StringIO()
    fieldnames = [
        "Phase_1_Total_Findings",
        "Phase_1_After_Dedup",
        "Vulnerability",
        "File",
        "Method",
    ...
    # Add header with phase statistics
    phase1_total = st.session_state.get("phase1_count", len(metrics))
    phase1_deduped = st.session_state.get("phase1_deduped", len(metrics))

    # Write header row with stats
    writer.writerow({
        "Phase_1_Total_Findings": phase1_total,
        "Phase_1_After_Dedup": phase1_deduped
    })
    ...
    for idx, row in enumerate(metrics, 1):
        output_row = {
            ...
            "OWASP_Category": row.get("OWASP Category", ""),
            "CWE_ID": row.get("CWE_ID", ""),
            "Phase_1_Scanner": row.get("Phase 1 Scanner", "Detected"),
            "Phase_2_RAG_Prediction": row.get("Phase 3 Prediction", ""),
    ...
    if not GROQ_API_KEY or GROQ_API_KEY == "PASTE_YOUR_API_KEY_HERE":
        ...
        with st.spinner("Initializing Pipeline..."):
            try:
                ...
                if is_sast:
                    # ==========================================
                    # SAST Pipeline Execution with Enhanced Error Handling
                    # ==========================================
                    try:
                        # ========== PHASE 1: Deterministic Scan + Triage ==========
                        status_placeholder.info("🔍 Phase 1: Running Deterministic SAST Engine (Semgrep)...")
                        raw_sast_data = run_semgrep(path, exclude_patterns=["**/codefixes/**", "**/*.info.yml"])
                        st.session_state.raw_json = raw_sast_data

                        if "error" in raw_sast_data:
                            error_msg = raw_sast_data.get("error", "Unknown error")
                            stderr_msg = raw_sast_data.get("stderr", "")
                            st.error(f"🚨 Phase 1 Failed: {error_msg}")
                            if stderr_msg:
                                with st.expander("📋 Semgrep Debug Output"):
                                    st.code(stderr_msg)
                        else:
                            # Parse semgrep findings
                            parsed_data = parse_semgrep_output(raw_sast_data)
                            st.session_state.phase1_count = len(parsed_data)
                            st.info(f"📊 Phase 1: Semgrep found {len(parsed_data)} raw findings")

                            # Deduplicate (still part of Phase 1)
                            status_placeholder.info(f"🔄 Phase 1: Deduplicating & triaging {len(parsed_data)} findings...")
                            try:
                                deduplicated_data = deduplicate_findings(parsed_data)
                                dup_count = len(parsed_data) - len(deduplicated_data)
                                st.session_state.phase1_deduped = len(deduplicated_data)
                                if dup_count > 0:
                                    st.info(f"📊 Removed {dup_count} duplicates")

                                # Scout Triage (lightweight, still Phase 1)
                                scout_report, filtered_data = run_scout_agent(deduplicated_data, GROQ_API_KEY)
                                st.session_state.security_report = scout_report
                                st.success(f"✅ Phase 1 Complete: {len(filtered_data)} findings ready for verification")

                            except Exception as phase1_err:
                                st.error(f"❌ Phase 1 Triage Error: {str(phase1_err)}")
                                st.warning("⚠️ Continuing with all raw findings")
                                filtered_data = parsed_data
                                st.session_state.phase1_deduped = len(filtered_data)

                            if not filtered_data:
                                status_placeholder.success("✅ Pipeline Complete! No findings to verify.")
                                st.session_state.verification_report = "No alerts survived Phase 1 triage."
                            else:
                                # ========== PHASE 2: RAG-Augmented Context Verification ==========
                                status_placeholder.info(f"🔐 Phase 2: RAG Verification — analyzing {len(filtered_data)} findings with repository context...")
                                try:
                                    verifier_report, metrics = run_verifier_agent(
                                        filtered_data,
                                    ...
                                    if metrics:
                                        validated = sum(1 for m in metrics if m.get("Phase 3 Prediction") == "VALIDATED")
                                        mitigated = sum(1 for m in metrics if m.get("Phase 3 Prediction") == "MITIGATED")
                                        errors = sum(1 for m in metrics if m.get("Phase 3 Prediction") == "Error")
                                        st.success(f"✅ Phase 2 Complete: {validated} VALIDATED, {mitigated} MITIGATED, {errors} errors")

                                except Exception as phase2_err:
                                    st.error(f"❌ Phase 2 Error: {str(phase2_err)}")
                                    st.warning("⚠️ RAG Verification failed - see raw findings below")
                                    st.session_state.verification_report = f"Phase 2 failed: {str(phase2_err)}"
                                    st.session_state.eval_metrics = []

                        status_placeholder.success("✅ Pipeline Execution Complete!")

                    except Exception as pipeline_err:
                        st.error(f"❌ Pipeline Error: {str(pipeline_err)}")
...
with right:
    if is_sast:
        ...
        if st.session_state.verification_report:
            st.subheader("✅ Phase 2: RAG-Augmented Context Verification")
            st.markdown(st.session_state.verification_report, unsafe_allow_html=True)
            st.divider()

            with st.expander("⚙️ View Phase 1: Deterministic Scan + Triage Details"):
                if st.session_state.security_report:
                    st.markdown("**Scout Triage Summary:**")
                    st.markdown(st.session_state.security_report)
                    st.divider()
                st.markdown("**Raw Semgrep Output:**")
            ...
            if st.session_state.eval_metrics:
                st.download_button(
                    ...
                    file_name=f"repomind_{repo_name}_eval.csv",
                    mime="text/csv",
                    use_container_width=True
                )
        elif st.session_state.raw_json and "error" in st.session_state.raw_json:
            st.subheader("🚨 Pipeline Halted — Semgrep Error")
            st.json(st.session_state.raw_json)
